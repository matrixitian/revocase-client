...
export default {
  ...
  methods: {
    ...
    async updateTradeURL() {
      ...
      const res = await axios.post('https://revo-cases.com/update-trade-url', {
    ...
    async sellSkin(skin, i) {
      const res = await axios.post('https://revo-cases.com/sell-skin', {
        skinID: skin._id
      })
    ...
    async requestTrade(skinID, i) {
      const res = await axios.post('https://revo-cases.com/request-trade', { skinID })
